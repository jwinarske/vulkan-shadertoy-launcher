...
static void check_hotkeys(struct app_os_window *os_window) {
    ...
    if (keyboard_map[Key_z][1]) {
        if (key_screenshot_once) {
            screenshot_once = true;
            key_screenshot_once = false;
        }
...
static void render_loop_init(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_swapchain *swapchain,
    ...
    if (!once) {
        ...
        retval =
                vk_get_presentable_queues(phy_dev, dev, swapchain->surface, &presentable_queues,
    ...
    retval =
            allocate_render_data(phy_dev, dev, swapchain, &essentials, &render_data,
    ...
    if (!once) {
        ...
        VkResult vk_res = vkCreateSemaphore(dev->device, &sem_info, NULL, &wait_buf_sem);
        ...
        vk_res = vkCreateSemaphore(dev->device, &sem_info, NULL, &wait_main_sem);
        vk_error_set_vkresult(&retval, vk_res);
        if (vk_res) {
        ...
        vk_res = vkCreateFence(dev->device, &fence_info, NULL, &offscreen_fence);
        vk_error_set_vkresult(&retval, vk_res);
        if (vk_res) {
...
void set_push_constants(struct app_os_window *os_window) {
    ...
    render_data.push_constants.iResolution[0] = static_cast<float>(os_window->app_data.iResolution[0]);
    render_data.push_constants.iResolution[1] = static_cast<float>(os_window->app_data.iResolution[1]);
    render_data.push_constants.iTime = os_window->app_data.iTime;
    render_data.push_constants.iTimeDelta = os_window->app_data.iTimeDelta;
    render_data.push_constants.iFrame = os_window->app_data.iFrame;
    render_data.push_constants.iMouse[0] = static_cast<float>(os_window->app_data.iMouse[0]);
    render_data.push_constants.iMouse[1] = static_cast<float>(os_window->app_data.iMouse[1]);
    render_data.push_constants.iMouse[2] = static_cast<float>(os_window->app_data.iMouse_lclick[0]);
    render_data.push_constants.iMouse[3] = static_cast<float>((last_iMousel_clicked[0]) ? -abs(
            os_window->app_data.iMouse_lclick[1])
                                                                                        : os_window->app_data.iMouse_lclick[1]);
    render_data.push_constants.iMouse_lr[0] = (int) os_window->app_data.iMouse_click[0];
    render_data.push_constants.iMouse_lr[1] = (int) os_window->app_data.iMouse_click[1];
    render_data.push_constants.iDate[0] = static_cast<float>(my_time.year);
    render_data.push_constants.iDate[1] = static_cast<float>(my_time.month);
    render_data.push_constants.iDate[2] = static_cast<float>(my_time.day);
    render_data.push_constants.iDate[3] = day_sec;
    render_data.push_constants.debugdraw = (int) os_window->app_data.drawdebug;
    render_data.push_constants.pCustom = (os_window->app_data.pause ? 1 : 0) + (main_image_srgb ? 10 : 0);
}
...
void update_push_constants_window_size(struct app_os_window *os_window) {
    ...
    render_data.push_constants.iMouse[1] = static_cast<float>(os_window->app_data.iMouse[1]);
    render_data.push_constants.iMouse[2] = static_cast<float>(os_window->app_data.iMouse_lclick[0]);
    render_data.push_constants.iMouse[3] = static_cast<float>(
            (last_iMousel_clicked[0]) ? -abs(os_window->app_data.iMouse_lclick[1])
                                      : os_window->app_data.iMouse_lclick[1]),
            render_data.push_constants.iResolution[0] = static_cast<float>(os_window->app_data.iResolution[0]);
    render_data.push_constants.iResolution[1] = static_cast<float>(os_window->app_data.iResolution[1]);
}
...
void update_push_constants_local_size(float width, float height) {
    render_data.push_constants.iMouse[0] =
            static_cast<float>(((render_data.push_constants.iMouse[0] / render_data.push_constants.iResolution[1]) -
                                0.5 * (render_data.push_constants.iResolution[0] /
                                       render_data.push_constants.iResolution[1])) *
                               height + 0.5 * width);
    render_data.push_constants.iMouse[1] = static_cast<float>(
    ...
    render_data.push_constants.iMouse[2] =
            static_cast<float>(sign(render_data.push_constants.iMouse[2]) *
                               (((fabs(render_data.push_constants.iMouse[2]) /
                                  render_data.push_constants.iResolution[1]) -
                                 0.5 * (render_data.push_constants.iResolution[0] /
                                        render_data.push_constants.iResolution[1])) *
                                height + 0.5 * width));
    render_data.push_constants.iMouse[3] =
            static_cast<float>(sign(render_data.push_constants.iMouse[3]) *
                               (((fabs(render_data.push_constants.iMouse[3]) /
                                  render_data.push_constants.iResolution[1]) - 0.5) * height +
                                0.5 * height));
...
static bool render_loop_draw(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_swapchain *swapchain,
    ...
    for (int i = 0; i < OFFSCREEN_BUFFERS; i++) {
        ...
        if (i == 0) { // wait main screen
            ...
            VkSubmitInfo submit_info = {
                    ...
                    .waitSemaphoreCount = first_submission ? UINT32_C(0) : UINT32_C(1),
    ...
    int result = vk_render_start(&essentials, dev, swapchain, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, &image_index);
    ...
    for (uint32_t i = 0; i < OFFSCREEN_BUFFERS; i++) {
        set_write_image_info[IMAGE_TEXTURES + i] = (VkDescriptorImageInfo) {
                .sampler = render_data.buf_obuffers[i * 2 + static_cast<uint32_t>(render_index)].color.sampler,
                .imageView = render_data.buf_obuffers[i * 2 + static_cast<uint32_t>(render_index)].color.view,
                .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        };
    }
    set_write_image_info[IMAGE_TEXTURES + OFFSCREEN_BUFFERS] = (VkDescriptorImageInfo) {
    ...
    vkUpdateDescriptorSets(dev->device, 3, set_write, 0, nullptr);

    vkCmdBindDescriptorSets(essentials.cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            render_data.main_layout.pipeline_layout, 0, 1, &render_data.main_desc_set, 0, nullptr);
    ...
    VkViewport viewport = {
            .x = 0,
            .y = 0,
            .width = static_cast<float>(render_data.main_gbuffers[image_index].surface_size.width),
            .height = static_cast<float>(render_data.main_gbuffers[image_index].surface_size.height),
            .minDepth = 0,
            .maxDepth = 1,
    };
    ...
    result = vk_render_finish(&essentials, dev, swapchain, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, image_index,
                              wait_buf_sem, wait_main_sem);
...
static void
render_loop_wayland(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_swapchain *swapchain,
                    struct app_os_window *os_window) {
    ...
    while (!os_window->app_data.quit) {
        ...
        if (((!os_window->is_minimized) && (!os_window->resize_event) &&
             ((!os_window->app_data.pause) || ((os_window->app_data.pause) && (os_window->pause_refresh))))
        ...
        if (os_window->is_minimized || (((os_window->app_data.pause) &&
                                         (!os_window->pause_refresh)))) { //I do not delete everything on minimize, only stop rendering
            sleep_ms(16);
        }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if ((strcmp(argv[i], "--present_mode") == 0) && (i < argc - 1)) {
            os_window.present_mode = (VkPresentModeKHR) atoi(argv[i + 1]);
            i++;
            continue;
        }
        if ((strcmp(argv[i], "--gpu") == 0) && (i < argc - 1)) {
            dev_index = (uint32_t) atoi(argv[i + 1]);
            use_gpu_idx = true;
            i++;
            continue;
        }
