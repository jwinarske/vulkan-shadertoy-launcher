...
VkResult vk_render_start(struct vk_render_essentials *essentials, struct vk_device *dev,
    ...
    res = vkAcquireNextImageKHR(dev->device, swapchain->swapchain, 1000000000, essentials->sem_post_acquire, NULL,
                                image_index);
    vk_error_set_vkresult(&retval, res);
    if (res == VK_TIMEOUT) {
...
static vk_error
fill_object(struct vk_device *dev, VkDeviceMemory to, void *from, size_t size, const char *object, const char *name) {
    void *mem = NULL;
    vk_error retval = VK_ERROR_NONE;
    VkResult res;
...
vk_error create_staging_buffer(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_buffer *staging,
...
vk_error vk_render_transition_images_mipmaps(struct vk_physical_device *phy_dev, struct vk_device *dev,
...
vk_error vk_render_update_texture(struct vk_physical_device *phy_dev, struct vk_device *dev,
                                  struct vk_render_essentials *essentials,
    ...
    retval = vk_render_transition_images(dev, essentials, image, 1, base_layout, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    retval = vk_render_copy_buffer_to_image(dev, essentials, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &staging,
                                            &image_copy, name);
    if (!vk_error_is_success(&retval))
        return retval;
    retval = vk_render_transition_images(dev, essentials, image, 1, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, base_layout,
                                         VK_IMAGE_ASPECT_COLOR_BIT, name);
    if (!vk_error_is_success(&retval))
        return retval;
...
vk_error vk_render_init_texture(struct vk_physical_device *phy_dev, struct vk_device *dev,
                                struct vk_render_essentials *essentials,
    ...
    retval = vk_render_transition_images(dev, essentials, image, 1, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    if (image->mipmaps) {
        ...
        if (retval.error.type == VK_ERROR_ERRNO)
            retval = vk_render_transition_images(dev, essentials, image, 1, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                                 layout, VK_IMAGE_ASPECT_COLOR_BIT, name);
    } else
        retval = vk_render_transition_images(dev, essentials, image, 1, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, layout,
                                             VK_IMAGE_ASPECT_COLOR_BIT, name);
...
vk_error vk_render_init_buffer(struct vk_physical_device *phy_dev, struct vk_device *dev,
                               struct vk_render_essentials *essentials,
