...
void registryGlobal(struct wl_registry *registry, uint32_t name, const char *interface, uint32_t /* version */,
    ...
    if (strcmp(interface, "wl_compositor") == 0) {
        ...
        xdg_wm_base_add_listener(os_window->shell, &xdg_wm_base_listener, nullptr);
...
static void
keyboardKeyCb(void *data, struct wl_keyboard *keyboard, uint32_t serial, uint32_t time, uint32_t key, uint32_t state) {
    auto os_window = static_cast<struct app_os_window *>(data);
...
static void seatCapabilities(struct wl_seat *seat, uint32_t caps, struct app_os_window *os_window) {
    if ((caps & WL_SEAT_CAPABILITY_POINTER) && !os_window->pointer) {
        ...
        static const struct wl_pointer_listener pointer_listener = {pointerEnterCb, pointerLeaveCb, pointerMotionCb,
        ...
        os_window->pointer = nullptr;
    }

    if ((caps & WL_SEAT_CAPABILITY_KEYBOARD) && !os_window->keyboard) {
        os_window->keyboard = wl_seat_get_keyboard(seat);
        static const struct wl_keyboard_listener keyboard_listener = {keyboardKeymapCb, keyboardEnterCb,
                                                                      keyboardLeaveCb, keyboardKeyCb,
                                                                      keyboardModifiersCb,};
